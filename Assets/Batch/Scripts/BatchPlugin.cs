...
namespace Batch
{
	...
	public class BatchPlugin : MonoBehaviour
	{
		...
		private Batch.Internal.Bridge bridge;
...
#pragma warning restore 612, 618
	    ...
	    void Awake()
	    {
			...
			bridge = new Batch.Internal.Bridge();
...
#pragma warning restore 612, 618
	    ...
	    void Start()
	    {
			...
			{
				Batch.Internal.Logger.Log(false, "Plugin Load", string.Format("{0} - Disabled because running in the editor or on an unsupported platform", VERSION));
			}
			else
			{
				Batch.Internal.Logger.Log(false, "Plugin Load", string.Format("{0} Loaded", VERSION));
			}
		}
...
#pragma warning restore 612, 618
		...
		public void StartPlugin(Config config)
		{
			...
			if (config == null)
			{
				Batch.Internal.Logger.Error(false, "StartPlugin", "Cannot start the plugin without a config.");
				return;
			}
			...
			{
				Batch.Internal.Logger.Error(false, "StartPlugin", "Cannot start the plugin without an API key.");
				return;
			}
			#endif
			...
			{
				Batch.Internal.Logger.Error(true, "StartPlugin", e);
			}
		}
		...
		private void setConfig(Config config)
		{
			...
			{
				Batch.Internal.Logger.Error(true, "setConfig", e);
			}
		}

		private bool isDevMode()
		{
			return Batch.Internal.Bridge.ResultToBool(bridge.Call("isDevMode", ""));
		}
		...
		void onBridgeFailure(string response)
		{
			...
			{
				Batch.Internal.Logger.Error(true, "onBridgeFailure", e);
			}
		}
